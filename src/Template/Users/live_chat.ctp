			...
			<div class="col-md-7"> 
				<div class="chat-screen-body">
					<div class="sweedyTeamColumn">
						<div id="chat" class="sweedyMsgouter" style="height: 500px;">
							<?php
							$page_num = $this->Paginator->current($model = null);
							if(!empty($ChatMessageslist)){
								foreach($ChatMessageslist as $msgs){ ?>	
						...
						<div class="sweedyChatMsg" id="message-container">
							...
							<input type="submit" id="send-message-live" value="Send" class="btn btn-danger btn-msg-send">
							<button id="login" class="btn btn-danger btn-msg-send" style="width: 150px;">Start Chat</button>
...
<?php $this->Html->scriptStart(['block' => true]); ?>
	...
	socket.on('receive message', function (msg) {
        ...
        if ($('#active_user_id').val() == FriendId) {
            ...
            $('.sweedyMsgouter').slimscroll({scrollBy: $('.sweedyMsgouter').prop("scrollHeight") + 'px'});
			//$('.sweedyMsgouter').scrollTop($('.sweedyMsgouter').scrollHeight);
        } else {
